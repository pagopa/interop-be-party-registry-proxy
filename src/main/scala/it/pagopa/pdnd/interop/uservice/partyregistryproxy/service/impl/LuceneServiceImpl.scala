...
final case class LuceneServiceImpl(pathDir: String) extends LuceneService {
  ...
  def searchByDescription(description: String, limit: Int): List[Institution] = {
    ...
    val hits: TopDocs           = searcher.search(query, limit)

    val results = hits.scoreDocs.toList.map(sc => searcher.doc(sc.doc).toInstitution)
    reader.close()
    results
  }
